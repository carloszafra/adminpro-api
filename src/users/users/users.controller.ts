...
export class UsersController {
  ...
  async updateUser(@Res() res: Response, @Body() userDto: edituserDto, @Param('id') userId: any, @Req() req: Request) {
    ...
    const userUpdated = await this.userSvc.updateUser(userId, userDto);
  ...
  async uploadUserImg(@Res() res: Response, @Req() req: Request, @UploadedFile() file: any ){
    const user = <JwtPayload>req.user;
    const userUpdated = await this.userSvc.updateUserImg(file.filename, user._id);
